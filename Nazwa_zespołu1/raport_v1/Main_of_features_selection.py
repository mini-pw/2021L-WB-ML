...
def features_selection():   
    ...
    name_dict = {'乳酸脱氢酶':'Lactate dehydrogenase (LDH)','淋巴细胞(%)':'Lymphocytes(%)','超敏C反应蛋白':'High-sensitivity C-reactive protein (hs-CRP)', '钠':'Sodium','氯':'Chlorine','国际标准化比值':'International Normalized Ratio (INR)','嗜酸细胞(#)':'Eosinophils(#)', '嗜酸细胞(%)':'Eosinophils(%)','单核细胞(%)':'Monocytes(%)','白蛋白':'Albumin'}
    #
    # 构建一个dataframe用于存储特征的重要程度信息
    import_feature = pd.DataFrame()
    import_feature['col'] = x_col
    import_feature['col_eng'] = list(map(name_dict.get, x_col))
    ...
    print(import_feature.iloc[:, 1:].head(10))
    ...
    g = sns.barplot(y=import_feature.iloc[:Num_f]['col_eng'].values[indices], x = import_feature.iloc[:Num_f]['xgb'].values[indices], orient='h') #import_feature.iloc[:Num_f]['col'].values[indices]
    ...
    while val_score_new >= val_score_old:
        ...
        print(list(map(name_dict.get, x_col)))
    ...
    print('Selected features:',list(map(name_dict.get, x_col[:-1])))
